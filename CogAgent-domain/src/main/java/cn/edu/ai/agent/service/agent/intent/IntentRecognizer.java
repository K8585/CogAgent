package cn.edu.ai.agent.service.agent.intent;
...
public class IntentRecognizer {
     ...
     * 基于规则的快速意图判断
     */
    private IntentResult ruleBasedRecognize(String userInput) {
        String lower = userInput.toLowerCase().trim();

        if (lower.matches("^(你好|hi|hello|嗨|早上好|下午好|晚上好).*")) {
        ...
        if (lower.contains("搜索") || lower.contains("查找") || lower.contains("最新") || lower.contains("最近")) {
            return IntentResult.builder()
                    .intentType("TOOL_USE")
                    .agentMode(AgentMode.REACT)
                    .confidence(0.8)
     ...
     * 基于 LLM 的意图识别（兜底方案）
     */
    private IntentResult llmBasedRecognize(String userInput) {
        try {
            String prompt = String.format(INTENT_PROMPT_TEMPLATE, userInput);
            String intentType = modelRouter.call(new Prompt(prompt), "deepseek-v4-flash")
                    .getResult().getOutput().getText().trim().toUpperCase();
            AgentMode agentMode = mapToAgentMode(intentType);
            boolean needsRag = "KNOWLEDGE_QA".equals(intentType);
            log.info("LLM 意图识别结果: type={}, mode={}", intentType, agentMode);
    ...
    private AgentMode mapToAgentMode(String intentType) {
        return switch (intentType) {
            ...
            case "REASONING" -> AgentMode.REFLECTION;
